...
public class English_MangaHere implements Source {
    ...
    private String findNextUrlFromParsedDocument(Document parsedDocument) {
        ...
        if (nextUrlElement != null) {
            return "http://www.mangahere.co" + nextUrlElement.attr("href");
        }
    ...
    public Observable<String> recursivelyConstructDatabase(final String url) {
        return MangaService.getInstance()
                ...
                .flatMap(new Func1<String, Observable<String>>() {
                    @Override
                    public Observable<String> call(String unparsedHtml) {
                        return Observable.just(parseEnglish_MangaHere(unparsedHtml));
                    }
                });
    }

    private static String INITIAL_DATABASE_URL = "http://www.mangahere.co/directory/?views.za";

    private static AtomicInteger mCounter = new AtomicInteger(0);

    private String parseEnglish_MangaHere(String unparsedHtml) {
        Document parsedDocument = Jsoup.parse(unparsedHtml);
        ...
        for (Element mangaElement : mangaElements) {
            ...
            String fieldSource = English_MangaHere.NAME;
        ...
        Element nextUrlElement = parsedDocument.select("a.next").first();
        if (nextUrlElement != null) {
            return "http://www.mangahere.co/directory/" + nextUrlElement.attr("href") + "?views.za";
        }
